...
function sd_edit_cod_amount($order_id)
{
    ...
    if (isset($_POST['shipdepot']['is_cod_edit']) && !Ship_Depot_Helper::check_null_or_empty(sanitize_text_field($_POST['shipdepot']['is_cod_edit'])) && sanitize_text_field($_POST['shipdepot']['is_cod_edit']) == 'true') {
        ...
        $rs = Ship_Depot_Helper::http_post($url, Ship_Depot_Helper::ParseObjectToArray($data_input), array('ShopAPIKey' => $shop_api_key));
...
function calculate_shipping_init()
{
    ...
    $data_input->courier = $setting_courier;
    //
...
if (!function_exists('sd_meta_boxes_content')) {
    ...
    ?>
        ...
        <div id="myModal" class="modal">
            <!-- Modal content -->
            <div class="modal-content" id="sd-modal-order-detail">
                <p><?php esc_html_e('Vui lòng chọn 1 trong những địa điểm dưới đây để tạo vận đơn Giao hàng tiết kiệm:', 'ship-depot-translate') ?></p>
                <select id="slGHTKHamlet">
                </select>
                <a id="btnModalOK" href="javascript:;" class="button-a"><?php esc_html_e('Đồng ý', 'ship-depot-translate') ?></a>
            </div>
        </div>
                        ...
                        ?>
                            ...
                            <div class="col-auto">
                                <div>
                                    ...
                                    ?>
                                    <p class="description"><?php esc_html_e('Thời gian nhận hàng ước tính:', 'ship-depot-translate') ?> <?php echo isset($ship_info->TimeExpected) ? esc_html($ship_info->TimeExpected) : ''; ?></p>
                                </div>
                            </div>
                        <?php
                        }
