...
class Xoo_El_Form_Handler{
	...
	public static function form_action(){
		...
		switch ($form_action) {
			...
			default:
				$action = array(
					'error' 	=> 0,
					'notice' 	=> xoo_el_add_notice( 'Không tìm thấy biểu mẫu', 'error' )
				);
		}
		...
		try {
			...
			if ( is_wp_error( $user ) ) {
				...
				$lost_pw_text = ' <a class="xoo-el-lostpw-tgr">' .
					__( 'Lấy lại mật khẩu?','easy-login-woocommerce' ).
					'</a>';
				...
				if( in_array( $message_code, $show_lostpw_on_errors ) ){
					$message  = __( 'Tên đăng nhập hoặc mật khẩu không đúng!.','easy-login-woocommerce' ).$lost_pw_text;
				}
				else{
				...
				$success_notice = apply_filters( 'xoo_el_login_success_notice', '<i class="fa fa-check-circle" aria-hidden="true"></i> '.__( 'Đăng nhập thành công!', 'easy-login-woocommerce' ), $user );
		...
		if ( isset($_POST['_xoo_el_form']) &&  $_POST['_xoo_el_form'] === 'register' ) {
			...
			try {
				...
				if ( empty( $password ) && $reg_admin_fields['xoo_el_reg_pass']['settings']['active'] === "yes" ) {
					throw new Xoo_Exception( __( 'Vui lòng nhập mật khẩu tài khoản.', 'easy-login-woocommerce' ) );
				}
				...
				if( $reg_admin_fields['xoo_el_reg_pass']['settings']['active'] === "yes" && $reg_admin_fields['xoo_el_reg_pass_again']['settings']['active'] === "yes" && $password !== $_POST['xoo_el_reg_pass_again'] ){
					throw new Xoo_Exception( __("Mật khẩu không khớp","easy-login-woocommerce") );
				}
				...
				if( self::$glSettings['m-auto-login'] === "yes" ){
					...
					$redirect = true;
					$success_notice = __( 'Đăng ký thành công', 'easy-login-woocommerce' );

				}
				else{
					$redirect = false;
					$success_notice = __( 'Đăng ký thành công. Vui lòng <a class="xoo-el-login-tgr">Đăng nhập</a>', 'easy-login-woocommerce' );
				}


				if( class_exists( 'woocommerce' ) && empty( $password ) ){
					$success_notice = $email ? __( 'Tài khoản của bạn đã được tạo thành công và mật khẩu đã được gửi đến địa chỉ email của bạn', 'easy-login-woocommerce' ) : __( 'Tạo tài khoản thành công.', 'easy-login-woocommerce' );
					$redirect = false;
				}
	...
	public static function create_customer( $email = '', $username = '', $password = '', $extra_data = array() ){
		...
		if ( !empty( $email ) ) {
			...
			if ( email_exists( $email ) ) {
				return new WP_Error( 'registration-error-email-exists',  __( 'Một tài khoản đã được đăng ký với địa chỉ email của bạn. Vui lòng hãy đăng nhập.', 'easy-login-woocommerce' ) );
			}

			if( !is_email( $email ) ){
				return new WP_Error( 'registration-error-invalid-email', __( 'Vui lòng cung cấp một địa chỉ email hợp lệ.', 'easy-login-woocommerce' ) );
			}
		...
		if ( !empty( $username ) ) {
			...
			if ( !validate_username( $username ) ) {
				return new WP_Error( 'registration-error-invalid-username', __( 'Vui lòng nhập tên người dùng tài khoản hợp lệ.', 'easy-login-woocommerce' ) );
			}

			if ( username_exists( $username ) ) {
				return new WP_Error( 'registration-error-username-exists', __( 'Một tài khoản đã được đăng ký với tên người dùng đó.', 'easy-login-woocommerce' ) );
			}
		} else {
		...
		if ( is_wp_error( $customer_id ) ) {
			return new WP_Error( 'registration-error', __( 'Couldn&#8217;t register you&hellip; vui lòng liên hệ với chúng tôi nếu bạn tiếp tục gặp vấn đề.', 'easy-login-woocommerce' ) );
		}
			...
			try{
				...
				if( class_exists( 'woocommerce' ) ){
					...
					if( is_wp_error( $success ) ){
						...
						$lost_password_confirmation = __( 'Email đặt lại mật khẩu đã được gửi đến địa chỉ email trong hồ sơ cho tài khoản của bạn. Nếu không có hãy kiểm tra trong hòm thư rác. Vui lòng đợi ít nhất 10 phút trước khi thử lại lần nữa. Pallmall cảm ơn!', 'easy-login-woocommerce' );
					}
				}
	...
	public static function process_reset_password(){
		...
		try {
			...
			if( is_wp_error( $user ) ){
				throw new Xoo_Exception( __( 'Khóa này không hợp lệ hoặc đã được sử dụng. Vui lòng đặt lại mật khẩu của bạn nếu cần.', 'easy-login-woocommerce' ) );
			}

			if ( $user instanceof WP_User ) {

				if( !isset( $_POST['xoo-el-rp-pass'] ) || !isset( $_POST['xoo-el-rp-pass-again'] ) || $_POST['xoo-el-rp-pass'] !== $_POST['xoo-el-rp-pass-again'] ){
					throw new Xoo_Exception( __( "Mật khẩu không đúng", 'easy-login-woocommerce' ) );
				}
				...
				$success_notice = apply_filters( 'xoo_el_reset_password_success_notice', __( 'Mật khẩu của bạn đã được đặt lại thành công! Vui lòng <a class="xoo-el-login-tgr">Login</a>', 'easy-login-woocommerce' ), $user );
	...
	public static function retrieve_password() {
		...
		if ( empty( $_POST['user_login'] ) || ! is_string( $_POST['user_login'] ) ) {
			$errors->add('empty_username', __('<strong>ERROR</strong>: Nhập tên người dùng hoặc địa chỉ email.','easy-login-woocommerce' ));
		} elseif ( strpos( $_POST['user_login'], '@' ) ) {
			$user_data = get_user_by( 'email', sanitize_email( trim( wp_unslash( $_POST['user_login'] ) ) ) );
			if ( empty( $user_data ) )
				$errors->add('invalid_email', __('<strong>ERROR</strong>: Không có người dùng nào đăng ký bằng địa chỉ email đó.','easy-login-woocommerce' ));
		} else {
		...
		if ( !$user_data ) {
			$errors->add('invalidcombo', __('<strong>ERROR</strong>: Tên người dùng hoặc email không hợp lệ.','easy-login-woocommerce' ));
			return $errors;
		}
		...
		$message = __( 'Ai đó đã yêu cầu đặt lại mật khẩu cho tài khoản sau:','easy-login-woocommerce' ) . "\r\n\r\n";
		...
		$message .= __( 'Nếu đây là sự nhầm lẫn, chỉ cần bỏ qua email này và sẽ không có gì xảy ra.','easy-login-woocommerce' ) . "\r\n\r\n";
	...
	public static function newuser_notification_email( $wp_new_user_notification_email, $user, $blogname ){
		...
		$message .= __( 'Để đặt mật khẩu, hãy truy cập địa chỉ sau:', 'easy-login-woocommerce' ) . "\r\n\r\n";
